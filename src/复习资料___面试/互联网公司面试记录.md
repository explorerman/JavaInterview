...
1. 算法题：旋转矩阵
2. 算法题：1.遍历一次字符串，选择出在该字符串中出现次数最多的字符及其出现次数
3. 设计题：用java代码实现典型的生成者、消费者场景，要求考虑到多线程并发的情况、缓冲区满的情况
4. 组合问题：输入一个字符串，输出该字符串中字符的所有组合。举个例子，如果输入abc，它的组合有a、b、c、ab、ac、bc、abc 
